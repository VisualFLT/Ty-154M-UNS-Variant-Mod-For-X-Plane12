...
-- controls
defineProperty("starter_cap", globalPropertyi("sim/custom/switchers/eng/starter_cap")) -- крышка панели запуска
defineProperty("starter_switch", globalPropertyi("sim/custom/switchers/eng/starter_switch")) -- выключатель запуска
defineProperty("starter_eng_select", globalPropertyi("sim/custom/switchers/eng/starter_eng_select")) -- выбор двигателя
defineProperty("starter_mode", globalPropertyi("sim/custom/switchers/eng/starter_mode")) -- режим запуска

defineProperty("starter_start", globalPropertyi("sim/custom/buttons/eng/starter_start")) -- кнопка запуска
defineProperty("starter_stop", globalPropertyi("sim/custom/buttons/eng/starter_stop")) -- преркащения запуска

defineProperty("flight_start_1", globalPropertyi("sim/custom/buttons/eng/flight_start_1")) -- запуск в полете
defineProperty("flight_start_2", globalPropertyi("sim/custom/buttons/eng/flight_start_2")) -- запуск в полете
defineProperty("flight_start_3", globalPropertyi("sim/custom/buttons/eng/flight_start_3")) -- запуск в полете
...
defineProperty("eng_airvalve_1", globalPropertyf("sim/custom/bleed/eng_airvalve_1")) -- открытие отбора воздуха от двигателя
defineProperty("eng_airvalve_2", globalPropertyf("sim/custom/bleed/eng_airvalve_2")) -- открытие отбора воздуха от двигателя
defineProperty("eng_airvalve_3", globalPropertyf("sim/custom/bleed/eng_airvalve_3")) -- открытие отбора воздуха от двигателя
...
defineProperty("auto_tanks_turn", globalPropertyi("sim/custom/fuel/auto_tanks_turn")) -- 0 = none, 1 = 2, 2 = 2+3, 3 = 3, 4 = 4
defineProperty("fuel_flow_mode", globalPropertyi("sim/custom/switchers/fuel/fuel_flow_mode")) -- режим расходомера. ручное - автомат


-- caps
defineProperty("engine_caps", globalPropertyi("sim/custom/anim/engine_caps"))  -- чехлы и крышки
...
-- results
defineProperty("starter_pressure", globalPropertyf("sim/custom/start/starter_pressure")) -- двление в системе запуска

defineProperty("apd_working_1", globalPropertyf("sim/custom/start/apd_working_1")) -- работа системы запуска
defineProperty("apd_working_2", globalPropertyf("sim/custom/start/apd_working_2")) -- работа системы запуска
defineProperty("apd_working_3", globalPropertyf("sim/custom/start/apd_working_3")) -- работа системы запуска

defineProperty("start_sys_work", globalPropertyf("sim/custom/start/start_sys_work")) -- работа системы запуска

defineProperty("fuel_in_1", globalPropertyi("sim/custom/start/fuel_in_1")) -- подача топлива от системы запуска
defineProperty("fuel_in_2", globalPropertyi("sim/custom/start/fuel_in_2")) -- подача топлива от системы запуска
defineProperty("fuel_in_3", globalPropertyi("sim/custom/start/fuel_in_3")) -- подача топлива от системы запуска
...
asu_press = globalPropertyf("sim/asu/press")
...
function update()
	...
	if power_sys then 
		...
		if not eng1_starting and not eng2_starting and not eng3_starting and start_button then
			if eng_select == 1 then
	...
	if eng1_starting and not eng1_starting_air and power27L then
		-- turn on the starter
		if time_now - eng1_start_time <= START_SEQ_TIME then
			set(globalPropertyi("sim/operation/flight_model/override/air_start_1"), 1)
			commandBegin(starter_1)
			print("DEBUG [start_logic]: commandBegin(starter_1) handle runned = "..tostring(starter_1))
			print("DBG bleed:", get(apu_air_doors), get(apu_n1), get(starter_pressure))
		end 
	...
	if eng2_starting and not eng2_starting_air and power27R then
		...
		if time_now - eng2_start_time > 1 and time_now - eng2_start_time <= START_SEQ_TIME then
			...
			print("DEBUG [start_logic]: starter_1 handle = "..tostring(starter_2))
		end 
