...
-- controls
defineProperty("apu_main_switch", globalPropertyi("sim/custom/switchers/eng/apu_main_switch")) -- выключатель ВСУ
defineProperty("apu_start_mode", globalPropertyi("sim/custom/switchers/eng/apu_start_mode")) -- режим запуска ВСУ
defineProperty("apu_air_bleed", globalPropertyi("sim/custom/switchers/eng/apu_air_bleed")) -- переключение заслонки отбора воздуха. -1 - закрыть, 0 - нейтр, +1 - открыть
defineProperty("apu_start", globalPropertyi("sim/custom/buttons/eng/apu_start")) -- кнопка старта ВСУ
defineProperty("apu_stop", globalPropertyi("sim/custom/buttons/eng/apu_stop")) -- кнопка стоп ВСУ

-- internal DataRefs
defineProperty("apu_n1", globalPropertyf("sim/custom/eng/apu_n1")) -- обороты ВСУ
defineProperty("apu_oil_t", globalPropertyf("sim/custom/eng/apu_oil_t")) -- температура масла ВСУ
defineProperty("apu_oil_q", globalPropertyf("sim/custom/eng/apu_oil_q")) -- количество масла ВСУ
defineProperty("apu_oil_p", globalPropertyf("sim/custom/eng/apu_oil_p")) -- давление масла ВС
defineProperty("apu_egt", globalPropertyf("sim/custom/eng/apu_egt")) -- температура выходных газов ВСУ
...
defineProperty("apu_fuel_last", globalPropertyf("sim/custom/eng/apu_fuel_last")) -- остаток топлива в двигателе

-- fuel weight in tank 1
defineProperty("tank1_w", globalPropertyf("sim/flightmodel/weight/m_fuel[0]")) -- fuel weight

-- results
defineProperty("apu_air_press", globalPropertyf("sim/custom/eng/apu_air_press")) -- давление воздуха для запуска двигателей
defineProperty("apu_air_doors", globalPropertyf("sim/custom/eng/apu_air_doors")) -- положение створок для накачки воздуха
defineProperty("apu_fuel_p", globalPropertyf("sim/custom/eng/apu_fuel_p")) -- давление топлива ВСУ

defineProperty("apu_start_bus", globalPropertyf("sim/custom/elec/apu_start_bus")) -- напряжение в сети ВСУ
defineProperty("apu_start_cc", globalPropertyf("sim/custom/elec/apu_start_cc")) -- потребление тока стартером ВСУ
defineProperty("apu_start_seq", globalPropertyi("sim/custom/elec/apu_start_seq")) -- идет процесс запуска ВСУ
defineProperty("fuel_pumps_27_cc", globalPropertyf("sim/custom/elec/fuel_pumps_27_cc")) -- нагрузка на сеть 27в от топливных насосо

defineProperty("apu_doors", globalPropertyf("sim/custom/anim/apu_doors")) -- положение створок ВСУ. 0 - закрыты, 1 - открыты.

defineProperty("apu_burn_fuel", globalPropertyi("sim/custom/elec/apu_burning_fuel")) -- ВСУ работает и сжигает топливо

-- engine #2 bleed
defineProperty("eng_airvalve_2", globalPropertyf("sim/custom/bleed/eng_airvalve_2")) -- открытие отбора воздуха от двигателя
...
-- failures
defineProperty("apu_start_fail",globalPropertyi("sim/custom/failures/apu_start_fail")) -- отказ стартера
defineProperty("apu_gen_fail",globalPropertyi("sim/custom/failures/apu_gen_fail")) -- отказ генератора
defineProperty("apu_runtime",globalPropertyf("sim/custom/failures/apu_runtime")) -- время наработки
defineProperty("apu_fail_oilt",globalPropertyi("sim/custom/failures/apu_fail_oilt")) -- отказ по температуре масла
defineProperty("apu_fail_egt",globalPropertyi("sim/custom/failures/apu_fail_egt")) -- отказ по ТВГ
defineProperty("apu_fail_fuel_left",globalPropertyi("sim/custom/failures/apu_fail_fuel_left")) -- отказ пр остатку топлива в камере при запуске
defineProperty("apu_fail",globalPropertyi("sim/custom/failures/apu_fail")) -- отказ по наработке
defineProperty("apu_press_fail", globalPropertyi("sim/custom/failures/apu_press_fail")) -- отказ отбора воздуха от двигателя
...
if MASTER then
	...
	if (apd_work_time > 60 or RPM > 60) and not starter_worked then -- automatic starter disconnect
		apu_starter = 0
		starter_worked = true
	elseif apd_work_time < 60 and apd_work_time > 1 and not starter_worked then -- connect starter
		apu_starter = 1
	...
	if RPM > 92 or apd_work_time > 60 then 
		starter_worked = false
	end
	
	-- calculate fuel intro
	if RPM > 21 and apd_work_time < 60 and fuel_press > 0.8 and apu_starter == 1 then
	...
	if get(apu_stop) == 1 or (apd_work_time > 60 and RPM < 45) or power_apu < 5 then 
		apu_burning_fuel = 0
		apd_work_time = 100
		apu_starter = 0
	end		
